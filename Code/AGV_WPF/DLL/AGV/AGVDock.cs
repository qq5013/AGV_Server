...
namespace AGV_WPF.DLL.AGV
{
    ...
    public class AGVDock
    {
        ...
        protected bool LineInit(int docknum)
        {
            ...
            if (docknum > 0)
            {
                ...
                if (lineSql.rowcount > 0)
                {
                    ...
                    dockLine = new Int16[dockCount];//确定停靠区每个车位的线路
                    outDockLine = new Int16[dockCount];
                    for (int i = 0; i < lineSql.rowcount; i++)
                    {
                        if (!string.IsNullOrEmpty(lineSql.Rows[i]["RouteNum"].ToString()))
                        {
                            dockLine[i] = Convert.ToInt16(lineSql.Rows[i]["RouteNum"]);
                        }
                    }
                    flag = true;
                }
        ...
        /// 更新停靠区号
        /// </summary>
        /// <param name="wlLink">无线状态</param>
        /// <param name="dockNum">停靠区</param>
        /// <param name="worklineNum">生产区</param>
        /// <param name="markNum">地标</param>
        /// <returns></returns>
        public int UpdateDock(bool wlLink,int dockNum,byte worklineNum,int markNum)
        {
            if (!wlLink) { dockNum = 0; } //当前AGV不在线
            for (int i = 0; i < dockCount; i++)
            {
                //不在停靠区时判断是否进入停靠区
                if (dockNum == 0)
                {
                    if (IsIntoDockArea(worklineNum, markNum))
                    {
                        return dockNum;
                    }
                }
                ...
                {
                    if (IsOutDockArea(worklineNum, markNum))
                    {
                        return 0;
                    }
                }
